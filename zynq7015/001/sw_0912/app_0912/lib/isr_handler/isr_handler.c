...
void scu_timer_handle()
{
	...
	log_upload("SCU_TIMER:CURRENT_TIME--%02d:%02d:%02d\r\n",hour,min,sec);

}

void axi_timer_cnt0_handler(){
	static u8 hour = 9;
	static u8 min  = 43;
	static u8 sec  = 35;
	...
	//AXI_Timer_Reg_Intc_Clean(&Axi_Timer0_Reg,AXI_TIMER_CNT0);
	//Axi_Timer0_Reg.TCR0.Reg_Value = XTmrCtr_ReadReg( Axi_Timer_Inst.BaseAddress,AXI_TIMER_CNT0,XTC_TCR_OFFSET);

	log_upload("AXI_TIMER:CURRENT_TIME--%02d:%02d:%02d\r\n",hour,min,sec);
	AXI_Timer_Reg_Intc_Clean(&Axi_Timer0_Reg,AXI_TIMER_CNT0);
	
	//XTmrCtrStats StatsPtr;
	//XTmrCtr_GetStats(&Axi_Timer_Inst, &StatsPtr);
	//u32 options = XTmrCtr_GetOptions(&Axi_Timer_Inst,AXI_TIMER_CNT0);
	//XTmrCtr_ClearStats(&Axi_Timer_Inst);
	//XTmrCtr_Reset(&Axi_Timer_Inst,AXI_TIMER_CNT0);
